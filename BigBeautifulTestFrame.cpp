...
/*   Updated: 2023/01/13 22:18:40 by hdoo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BigBeautifulTestFrame::printMessageLine()
{
	...
	printColoredRawWall();
	std::cout.width(static_cast<long>(leftPedding + color_.strColor.length()));
	std::cout.fill(' ');
	std::cout << color_.strColor;
	printColoredRawMessage();
	std::cout.width(static_cast<long>(width_ - leftPedding + color_.frameColor.length() - message_.length() - 1));
	std::cout.fill(' ');
...
{
	printColoredRawWall();
	std::cout.width(static_cast<long>(width_ + color_.frameColor.length() - 1));
	std::cout.fill(' ');
	printColoredRawWall();
	std::cout << std::endl;
}
...
void BigBeautifulTestFrame::printVerticalFrame()
{
	...
	{
		std::cout.width(static_cast<long>(width_));
...
void printVerboseTitle(const std::string& str, const std::string& color, size_t padding)
{
#ifdef VERBOSE
	BigBeautifulTestFrame f;
...
void printVerboseTitleEnter(const std::string& str, const std::string& color, size_t padding)
{
#ifdef VERBOSE
	std::string enter;
...
std::string itoaStream(int i)
{
	...
	ss >> temp;
	return (temp);
	// return static_cast<std::ostringstream&>(std::ostringstream().operator<<(i)).str();
}
